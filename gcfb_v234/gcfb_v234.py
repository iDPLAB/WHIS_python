...
def gcfb_v234(snd_in, gc_param, *args):
    ...
    print("--- Channel-by-channel processing ---")#   t_est
...
def gcfb_v23_frame_base(pgc_smpl, scgc_smpl, gc_param, gc_resp):# t_est
...
def gcfb_v23_synth_snd(gc_smpl, gc_param):
    ...
    if not 'NO' in gc_param.out_mid_crct:
        ...
        inv_cmpn_out_mid, _ = utils.mk_filter_field2cochlea(gc_param.out_mid_crct, fs, -1)
        ...
        snd_syn1_time = list(snd_syn1[n_delay:]) + np.zeros(n_delay, dtype=np.complex128).tolist()
        snd_syn = amp_syn * np.array(snd_syn1_time, dtype=np.complex128).real
